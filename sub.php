...
<html lang="en">
  ...
  @media screen and (min-device-width: 771px) and (max-device-width: 1400px){
    .imge{
    height:400px;
    width:680px;
    border-radius:20px;
  ...
  <?php include 'partials/_header.php'; ?>
    ...
    <div class="container">
      ...
      <h2 class="z my-2">Please select your Subject</h2>
      <div class="container my-5">

      <?php
 $medium = $_GET['medium'];
    $std = $_GET['std'];
    $sql = "SELECT DISTINCT `subject`,`std`,`medium` FROM `que_ans` WHERE `medium`= '$medium' and `std`= '$std'"; 
    $result = mysqli_query($conn, $sql);
    while($row = mysqli_fetch_assoc($result)){
        $subject= $row['subject'];
        echo '
        <a href="chap.php?std='.$std.'&& medium='.$medium.'&& subject='.$subject.'"><button type="button" id="btn" class="btn btn-outline-success mx-1 my-1"  >'.$subject.'</button></a> 
        ';
